...
void PaletteTransform::set(uchar* _from,uchar* _to,int _n0,int _sz,int* _event) {
	...
	if (!_from) { memset(from + 3*n0, 0, 3*sz); }
	else { memcpy(from + 3*n0, _from + 3*n0, 3*sz); }
	
	if (!_to) { memset(to + 3*n0, 0, 3*sz); }
	else { memcpy(to + 3*n0, _to + 3*n0, 3*sz); }
	
	memset(delta + 3*n0, 0, 3*sz);
	
	for(int i = 0, j = 3*n0; i < 3*sz; i++, j++) {
...
int PaletteTransform::quant(void) {
	...
	while (max--) {
		if(*pd) {
			log = 1;
			if (abs((int)*pf - (int)*pt) <= abs(*pd)) { *pf = *pt, *pd = 0; }
			else { *pf += *pd; }
		}
		pf++; pt++; pd++;
	}
...
void pal_iter_num(int pal_num) {
	if (PAL_WAVE_TERRAIN == -1 || PAL_WAVE_TERRAIN >= TERRAIN_MAX) { return; }
	
	int DSIZE, PRD;
	if (!pal_num) { DSIZE = 8; PRD = 64; }
	else { DSIZE = 10; PRD = 150; }
	
	int *DATA;
	if (!pal_num) { DATA = new int[DSIZE] {1, 3, 5, 6, 5, 3, 2, 1}; } 
...
void pal_iter2(void) {
	...
	for (int ind = 0; ind < PAL_MAX; ind++) {
		...
		for (int i = BEGCOLOR[PAL_TERRAIN[ind]]; i <= ENDCOLOR[PAL_TERRAIN[ind]]; i++) {
			if (PAL_RED[ind]) {
				j = *po + add; 
				if (j > 255) { j = 255; } 
				else if (j < 0) { j = 0; }
