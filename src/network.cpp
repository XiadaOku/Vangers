...
ServerFindChain::ServerFindChain(int IP,int port,char* domain_name,int game_ID,char* game_name)
{
	...
	if(!game_ID) {
		switch(RND(40)) {
			case 0: new_game_name = (char*)"Аид, когда CX на "; break;
			case 1: new_game_name = (char*)"Поставить линукс на "; break;
			case 2: new_game_name = (char*)"Убить Димона на "; break;
			case 3: new_game_name = (char*)"Использовать инкарнатор на "; break;
			case 4: new_game_name = (char*)"Положить сервер "; break;
			case 5: new_game_name = (char*)"Удалить сервер "; break;
			case 6: new_game_name = (char*)"Виден коридор на "; break;
			case 7: new_game_name = (char*)"Ксиаде нечего делать на "; break;
			default: new_game_name = (char*)"Новая игра на "; break;		
		}
...
//XnB TODO shellnetid in messages
	...
	else if (!is_kill && (strcmp(msg, "/kill") == 0 || strcmp(msg, ".лшдд") == 0)) {
		*name = *player_name;
        actual_msg = msg;
        actual_col = col;
	...
	else if (start_status == START_STATUS::NONE && (strcmp(msg, "/start") == 0 || strcmp(msg, ".ыефке") == 0)) {
		*name = *bot_name;
		
		if  (isMod(MOD_ID::MAMMOTH)) {
			//XnB TODO config file for mods or just timer but anyway 
			actual_msg = "Старт мамонта через 20 секунд, охотников через 40";
		} 
		else {
			actual_msg = "Старт через 20 секунд";
		}
	...
	else if ((start_status == START_STATUS::INGAME || start_status == START_STATUS::DROPPED_OUT) && (strcmp(msg, "/finish") == 0 || strcmp(msg, ".аштшыр") == 0)) {
		*name = *bot_name;
		actual_msg = "Финиш";
		actual_col = bot_color;
	...
	//XnB TODO own rollcall messages
	else if (start_status == START_STATUS::INGAME && kvach_status == KVACH_STATUS::GET && (strcmp(msg, "я") == 0 || strcmp(msg, "z") == 0 || strcmp(msg, "Я") == 0|| strcmp(msg, "Z") == 0)) {
		...
		*actual_msg = *player_name;
		...
		strcat(actual_msg, " готов");
		actual_col = bot_color;
		
		kvach_id = atoi(msg + strlen("/kvach"));
	}
	else if (start_status == START_STATUS::INGAME && isMod(MOD_ID::MECHOKVACH) && (strcmp(msg, "/rekvach") == 0 || strcmp(msg, ".кулмфср") == 0)) {
		*name = *bot_name;
		actual_msg = "Переквачка";
		actual_col = bot_color;
		
		kvach_status = KVACH_STATUS::GET;
	}
	else if ((strncmp(msg, "/mess ", 6) == 0 || strncmp(msg, ".ьуыы ", 6) == 0) && strncmp(player_name, "xiada", 5) == 0) {
		name = "";
		actual_msg = "";
		actual_col = bot_color;
		...
		if (message_str != "--old" || message_str != "--щдв") {
	...
	else if (!is_check && strcmp(msg, "/check") == 0 || strcmp(msg, ".срусл") == 0) {
		*name = *player_name;
        actual_msg = msg;
        actual_col = col;
	...
	else if (!is_rollcall && (strcmp(msg, "/rollcall") == 0 || strcmp(msg, ".кщддсфдд") == 0)) {
		*name = *bot_name;
		actual_msg = "Перекличка";
		actual_col = bot_color;
	...
	else if (is_rollcall && (strcmp(msg, "/rcancel") == 0 || strcmp(msg, ".ксфтсуд") == 0)) {
		*name = *bot_name;
		actual_msg = "Перекличка отменена";
		actual_col = bot_color;

		is_rollcall = false;
		rollcall_nicknames.clear();
	} 
	else if (start_status == START_STATUS::TIMER && (strcmp(msg, "/scancel") == 0 || strcmp(msg, ".ысфтсуд") == 0)) {
		*name = *bot_name;
		actual_msg = "Старт отменен";
		actual_col = bot_color;

		start_status = START_STATUS::NONE;
	} 
	else if (is_rollcall && (strcmp(msg, "я") == 0 || strcmp(msg, "z") == 0 || strcmp(msg, "Я") == 0 || strcmp(msg, "Z") == 0)) {
		*name = *player_name;
        actual_msg = msg;
        actual_col = col;
...
void MessageDispatcher::send(char* message,int mode,int parameter)
{
	...
	if ((strncmp(message, "/mess ", 5) == 0 || strncmp(message, ".ьуыы ", 6) == 0) && strncmp(CurPlayerName, "xiada", 5) == 0) {
