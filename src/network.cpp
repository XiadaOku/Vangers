...
ServerFindChain::ServerFindChain(int IP,int port,char* domain_name,int game_ID,char* game_name)
{
	...
	if(!game_ID) {
		switch(RND(20)) {
			case 1: new_game_name = "€¨¤, ª®£¤  CX ­  "; break;
			case 2: new_game_name = "®§¢ âì ”…Š  ­  "; break;
			case 3: new_game_name = "®áâ ¢¨âì «¨­ãªá ­  "; break;
			case 4: new_game_name = "“¡¨âì „¨¬®­  ­  "; break;
			case 5: new_game_name = "‡ ¡ëâì ¯à® áâ àâ ­  "; break;
			case 6: new_game_name = "‚§ïâì ¢¥ªâ®à ­  "; break;
			case 7: new_game_name = " ¯¨á âì £®¢­®ª®¤ ­  "; break;
			case 8: new_game_name = "‘¤¥« âì ª®áâë«¨ ­  "; break;
			case 9: new_game_name = "®«®¦¨âì á¥à¢¥à "; break;
			case 10: new_game_name = "“¤ «¨âì á¥à¢¥à "; break;
			case 11: new_game_name = "Ž¡®£­ âì â®à£ è  ­  "; break;
			case 12: new_game_name = "„¥¡ ¦¨âì ­  "; break;
			case 13: new_game_name = "®«ãç¨âì ¡ ­ ­  "; break;
			case 14: new_game_name = "à¨§¢ âì á¢¨­  ­  "; break;
			case 15: new_game_name = "‚¨¤¥­ ª®à¨¤®à ­  "; break;
			default: new_game_name = "Šá¨ ¤¥ ­¥ç¥£® ¤¥« âì ­  "; break;		
		}
		str_buf < new_game_name;
	}
	if (RND(40) == 1) str_buf < "ara ara";
	else {
		if(!game_name)
			if(domain_name)
				str_buf < domain_name;
...
int InputEventBuffer::receive_waiting_for_event(int event, XSocket& sock,int skip_if_aint)
{
	...
	if(!skip_if_aint)
        {
	    if (lang() == RUSSIAN) {
            ErrH.Abort("Ð ÐŽÐ ÂµÐ¡Ð‚Ð Ð†Ð ÂµÐ¡Ð‚ Ð Ð…Ð Âµ Ð Ñ•Ð¡â€šÐ Ð†Ð ÂµÐ¡â€¡Ð Â°Ð ÂµÐ¡â€š", XERR_USER, event);
        } else {
...
int InputEventBuffer::next_event() {
	...
	*this > event_ID;
	...
	if (event_ID == zCREATE_OBJECT_BY_SERVER) {
		...
		//zmod - Ð Ñ—Ð Â°Ð Ñ”Ð ÂµÐ¡â€š "Ð¡ÐƒÐ Ñ•Ð Â·Ð Ò‘Ð Â°Ð â„– Ð Ñ—Ð¡Ð‚Ð ÂµÐ Ò‘Ð Ñ˜Ð ÂµÐ Ò‘"
...
int connect_to_server(ServerFindChain* p)
{
	...
	if(main_socket() && identification(main_socket)){
		...
		if (strcmp(iScrOpt[iSERVER_NAME]->GetValueCHR(),"neptune")==0) {
			message_dispatcher.send("[bot]ï¿½ï¿½ï¿½ï¿½ï¿½ - ï¿½ï¿½ï¿½-ï¿½ï¿½ï¿½ ï¿½ï¿½ ï¿½ï¿½ï¿½ï¿½ï¿½ ï¿½ï¿½ ï¿½ï¿½ï¿½á®¢ï¿½", MESSAGE_FOR_PLAYER, 0);
			message_dispatcher.send("[bot]ï¿½ï¿½ ï¿½ï¿½å®¤ï¿½ï¿½ ï¿½ ï¿½ï¿½ï¿½, ï¿½ï¿½ï¿½ã¯ ï¿½ï¿½ á­ ï¿½ï¦¥ï¿½ï¿½ï¿½", MESSAGE_FOR_PLAYER, 0);
			message_dispatcher.send("[bot]ï¿½ ï¿½ï¿½ï¿½à ¢ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ ï¿½ ï¿½ï¿½à¨¤ï¿½ï¿½ï¿½ ï¿½ï¿½ ï¿½ï¿½ï¿½á®¢", MESSAGE_FOR_PLAYER, 0);
			message_dispatcher.send("[bot]ï¿½ï¿½á«¥ ï¿½ï¿½ï¿½ï¿½ ï¿½ï¿½ ï¿½ï¿½ï¿½å®¤ï¿½ï¿½ ï¿½ï¿½ ï¿½ï¿½ï¿½á®¢ ï¿½ ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ ï¿½ï¿½ï¿½", MESSAGE_FOR_PLAYER, 0); 
		}
...
int restore_connection()
{
	...
	if(!main_socket){
		if(number_of_reconnection_attempt-- <= 0)
            {
		    if (lang() == RUSSIAN) {
                ErrH.Abort("Ð ÑœÐ Âµ Ð Ñ˜Ð Ñ•Ð Ñ–Ð¡Ñ“ Ð Ð†Ð Ñ•Ð¡ÐƒÐ¡ÐƒÐ¡â€šÐ Â°Ð Ð…Ð Ñ•Ð Ð†Ð Ñ‘Ð¡â€šÐ¡ÐŠ Ð¡ÐƒÐ Ñ•Ð ÂµÐ Ò‘Ð Ñ‘Ð Ð…Ð ÂµÐ Ð…Ð Ñ‘Ð Âµ Ð¡Ðƒ Ð ÐŽÐ ÂµÐ¡Ð‚Ð Ð†Ð ÂµÐ¡Ð‚Ð Ñ•Ð Ñ˜");
            } else {
...
MessageElement::MessageElement(const char* player_name, char* msg,int col)
{
    ...
    if (strncmp(msg, bot_tag, 5)==0) {
	...
	else if ((strncmp(msg, "/mess", 5)==0 || strncmp(msg, ".ìãëë", 5)==0) && strncmp((char*)player_name, "xiada", 5)==0) {
		name = (char*)"$";
		...
		spamN = atoi(spamNum);
		if (strcmp((char*)spamMsg, (char*)"-old") != 0 && strcmp((char*)spamMsg, (char*)"-é¤¢") != 0) {
	...
	else if (strcmp(msg, "/check")==0||strcmp(msg, ".áàãá«")==0) {
		name = (char*)player_name;
        actual_msg = msg;
        actual_col = col;
		checkModVersion = 1;
...
void MessageDispatcher::send(char* message,int mode,int parameter)
{
	...
	if ((strncmp(message, "/mess", 5)==0 || strncmp(message, ".ìãëë", 5)==0) && strncmp(CurPlayerName, "xiada", 5)==0)
		cors = 1 << (parameter - 1);
